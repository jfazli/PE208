...
public class AssimetricWindEffect implements Effect {
	 ...
	 *            number of rows > 0
	 */
	private AssimetricWindEffect(boolean appear, boolean leftToRight,
	...
	public AssimetricWindEffect(EffectData edata) {

		AssimetricWindEffectData data = (AssimetricWindEffectData) edata;
		...
		this.tiles = Util.getTilesForWindEffect(image, this.cols, this.rows,
				data.isAppear(), data.getPhases(), data.getFactor());
	 ...
	 */
	public boolean draw(GraphicsContext gc, int _x, int _y, int width,
			int height) {

		gc.setClip(_x, _y, width, height);
		...
		int globalTranslateX = _x - 1;
		int globalTranslateY = _y - 1;
		...
		for (int i = 0; i < tiles.size(); i++) {
			...
			// prepare position for the next iteration
			if (tile.getMovingSteps() > 0) {
				float x = tile.getAx();
				float y = tile.getAy();
				...
				tile.setX((int) (x + 0.5));
				tile.setY((int) (y + 0.5));
				tile.setMovingSteps(tile.getMovingSteps() - 1);
			}
