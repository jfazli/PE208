...
//#define LCD_FRAME_BUFFER       ((uint32_t)0x64000000)

#define BUFFER_OFFSET          LCD_PIXEL_HEIGHT * LCD_PIXEL_HEIGHT * 4 //((uint32_t)0x130000)
...
void LCD_WriteSDRAM(u16* LCD_Ptr,uint16_t RGB_Code)
{
  *LCD_Ptr = RGB_Code;
}
...
{ 
 GPIO_InitTypeDef GPIO_InitStructure;
  ...
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0  | GPIO_Pin_1  | GPIO_Pin_2  | 
                                ...
                                GPIO_Pin_11 | GPIO_Pin_10 | GPIO_Pin_12 | 
  ...
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0 | GPIO_Pin_1  | GPIO_Pin_2  | 
                                ...
                                GPIO_Pin_7 | GPIO_Pin_12 | GPIO_Pin_13 | 
                                GPIO_Pin_15;      
...
{ 
  ...
  */      
  ...
  LTDC_Layer_InitStruct.LTDC_VarticalStart = 35;
  ...
  */ 
  ...
  //phv 8/02LTDC_LayerInit(LTDC_Layer2, &LTDC_Layer_InitStruct);

#endif /* USE_LCD_AM640480G5TNQW */
  ...
  /* Enable Pixel Clock */
  /* PLLSAI_VCO Input = HSE_VALUE/PLL_M = 1 Mhz */
  /* PLLSAI_VCO Output = PLLSAI_VCO Input * PLLSAI_N = 192 Mhz        216Mhz*/
  /* PLLLCDCLK = PLLSAI_VCO Output/PLLSAI_R = 192/3  = 64 Mhz         72Mhz */
  /* LTDC clock frequency = PLLLCDCLK / RCC_PLLSAIDivR = 64/8 = 8 Mhz 72/8 = 9 Mhz */
  RCC_PLLSAIConfig(216, 7, 3); //192/7/3
  ...
  /* Timing configuration */  
  /* Configure horizontal synchronization width */     
  LTDC_InitStruct.LTDC_HorizontalSync = 9;						//10 - 1
  /* Configure vertical synchronization height */
  LTDC_InitStruct.LTDC_VerticalSync = 9;							//10 - 1
  /* Configure accumulated horizontal back porch */
  LTDC_InitStruct.LTDC_AccumulatedHBP = 42; 					//43 - 1
  /* Configure accumulated vertical back porch */
  LTDC_InitStruct.LTDC_AccumulatedVBP = 11;  					//12 - 1
  /* Configure accumulated active width */  
  LTDC_InitStruct.LTDC_AccumulatedActiveW = 523;			//480 + 43 - 1
  /* Configure accumulated active height */
  LTDC_InitStruct.LTDC_AccumulatedActiveH = 283;			//272 + 12 - 1
  /* Configure total width */
  LTDC_InitStruct.LTDC_TotalWidth = 530; 							//531 - 1
  /* Configure total height */
  LTDC_InitStruct.LTDC_TotalHeigh = 287;							//287 - 1
  ...
  */      
  ...
  /* Pixel Format configuration*/            
  LTDC_Layer_InitStruct.LTDC_PixelFormat = LTDC_Pixelformat_RGB565 ;// LTDC_Pixelformat_RGB565;
  /* Alpha constant (255 totally opaque) */
  LTDC_Layer_InitStruct.LTDC_ConstantAlpha = 255; 
  /* Default Color configuration (configure A,R,G,B component values) */          
  LTDC_Layer_InitStruct.LTDC_DefaultColorBlue = 0;        
  ...
  */ 
  ...
  /* phv Enable foreground & background Layers */
  ...
  LTDC_Cmd(ENABLE);
	...
	LCD_Clear();
	LCD_ClearOriginal(LCD_COLOR_BLACK);
	for(i = 0; i < 0x1000000; i++)
	{
		;
	}
	
	LCD_ClearOriginal(LCD_COLOR_BLUE);
	for(i = 0; i < 0x1000000; i++)
	{
		;
	}
	
	LCD_ClearOriginal(LCD_COLOR_RED);
	for(i = 0; i < 0x1000000; i++)
	{
		;
	}
	
	LCD_ClearOriginal(LCD_COLOR_GREEN);
	for(i = 0; i < 0x1000000; i++)
	{
		;
	}
  ...
  LCD_ClearOriginal(LCD_COLOR_WHITE);
...
void LCD_Clear(void)
{
  ...
  DMA2D_InitStruct.DMA2D_OutputGreen = (0x07E0 & CurrentBackColor) >> 5;      
  DMA2D_InitStruct.DMA2D_OutputBlue = 0x001F & CurrentBackColor;     
  DMA2D_InitStruct.DMA2D_OutputRed = (0xF800 & CurrentBackColor) >> 11;                
...
{
  __IO uint32_t index = 0, counter = 0, xpos =0;
  __IO uint32_t  Xaddress = 0;
  
  xpos = Xpos*LCD_PIXEL_WIDTH*2*LCD_Currentfonts->Height;
  Xaddress += Ypos;
...
{  
  ...
  //while ((*ptr != 0) & (((refcolumn + 1) & 0xFFFF) >= LCD_Currentfonts->Width))
	while ((*ptr != 0) & (((refcolumn) & 0xFFFF) <= LCD_PIXEL_WIDTH - LCD_Currentfonts->Width))
  {
    /* Display one character on LCD */
...
/*
 ...
 |  LCD_TFT R2 <->        |  LCD_TFT G2 <-> PA.08 |  LCD_TFT B2 <->            |
 ...
 -------------------------------------------------------------------------------
          ...
          |  LCD_TFT CLK   <-> PG.07  | LCD_TFT DE   <->  PF.10 |
           -----------------------------------------------------
          |LCD_TFT backlight <-> PC.08| LCD_CSB <-> PG.13    |
           -----------------------------------------------------
*/
 ...
 /* GPIOA configuration */
  ...
  GPIO_InitStruct.GPIO_Pin = GPIO_Pin_3 | GPIO_Pin_4 | GPIO_Pin_6 | GPIO_Pin_8 | GPIO_Pin_11 | GPIO_Pin_12;
 ...
 /* GPIOB configuration */
	//R3
  GPIO_PinAFConfig(GPIOB, GPIO_PinSource0, GPIO_AF_LTDC2);				//AF9?
	//B6
  ...
  GPIO_InitStruct.GPIO_Pin = GPIO_Pin_0 | GPIO_Pin_8 | GPIO_Pin_8 | GPIO_Pin_10 | \
                             GPIO_Pin_11 ;
 ...
 /* GPIOC configuration */
  ...
  GPIO_Init(GPIOF, &GPIO_InitStruct);
	...
	/* GPIOG configuration */
  //B4
	GPIO_PinAFConfig(GPIOG, GPIO_PinSource12, GPIO_AF_LTDC2);				//AF9?
	//B3
  ...
  /*  GPIOC configuration Backlight control */ 
