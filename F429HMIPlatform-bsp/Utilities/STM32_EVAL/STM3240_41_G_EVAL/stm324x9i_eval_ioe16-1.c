...
/** @defgroup STM324x9I_EVAL_IOE16_Private_Defines
  * @{
  */ 
#define TIMEOUT_MAX    0x10000 /*<! The value of the maximal timeout for I2C waiting loops */
/**
  * @}
  */ 
...
/** @defgroup STM324x9I_EVAL_IOE16_Private_FunctionPrototypes
 ...
 void IOE16_GPIO_Config(void);
 void IOE16_I2C_Config(void);
 void IOE16_EXTI_Config(void);
...
/**
  * @brief  Monitors the status of the selected input IO pin.
  ...
  */
uint8_t IOE16_MonitorIOPin(uint16_t IO_Pin, uint8_t RegisterAddr)
{
  uint8_t tmp = 0;
  ...
  if(IO_Pin & IO16_Pin_ALL_LSB)
  {
    ...
    tmp = IOE16_I2C_ReadDeviceRegister(RegisterAddr);
...
/**
  * @brief  Enable or disables polarity inversion of the selected IO pins.
  * @param IO_Pin: The input pin to be read. This parameter could be any
  *                of the following values:
  *   @arg  IO_Pin_x: where x can be from 0 to 15.
  * @param NewState: The value to be set. This parameter can be one of the
...
uint16_t IOE16_GetITStatus(void)
{
  ...
  /* Gets the IT status register value */
...
uint8_t IOE16_ITOutConfig(uint8_t Polarity)
{
  ...
  /*  Get the register IOE16_REG_INT_CTRL value */ 
...
uint8_t IOE16_IOPinConfig(uint16_t IO_Pin, uint8_t Direction)
{
  ...
  IOE16_I2C_WriteDeviceRegister(IOE16_REG_GPDR_MSB, (uint8_t)((tmp & IO16_Pin_ALL_LSB)>>8));
...
uint16_t IOE16_I2C_ReadDataBuffer(uint32_t RegisterAddr)
{
  ...
  Buffer[1] = I2C_ReceiveData(IOE16_I2C);
  ...
  Buffer[0] = I2C_ReceiveData(IOE16_I2C);
...
void IOE16_EXTI_Config(void)
{
  ...
  /* Connect TS + JoyStick EXTI Line to  GPIO Pin */
  SYSCFG_EXTILineConfig(IOE16_IT_EXTI_PORT_SOURCE, IOE16_IT_EXTI_PIN_SOURCE);  
  
  /* Configure TS + JoyStick EXTI line */
