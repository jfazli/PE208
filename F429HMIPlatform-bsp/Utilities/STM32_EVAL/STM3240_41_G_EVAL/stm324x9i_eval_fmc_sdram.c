...
/* FMC SDRAM Bank configuration */   
  ...
  FMC_SDRAMTimingInitStructure.FMC_WriteRecoveryTime = 1;      
  /* TRP:  20ns => 2x11.90ns */
...
/*
 ...
 +-------------------+--------------------+--------------------+
 | PC2 <-> FMC_SDCKE0| PI4 <-> FMC_NBL2   |
 | PC3 <-> FMC_SDNE0 | PI5 <-> FMC_NBL3   |
 | PC0 <-> FMC_SDNW  |--------------------+
 +-------------------+
 +-------------------+------------------+
...
/**
  ...
  * @retval None.
  */
void SDRAM_WriteBuffer(uint16_t* pBuffer, uint32_t uwWriteAddress, uint32_t uwBufferSize)
{
  ...
  for (; uwBufferSize != 0; uwBufferSize--) 
  {
    ...
    *(uint16_t *) (SDRAM_BANK_ADDR + write_pointer) = *pBuffer++;
...
/**
  ...
  * @retval None.
  */
void SDRAM_ReadBuffer(uint16_t* pBuffer, uint32_t uwReadAddress, uint32_t uwBufferSize)
{
  ...
  {
   *pBuffer++ = *(__IO uint16_t *)(SDRAM_BANK_ADDR + write_pointer );
